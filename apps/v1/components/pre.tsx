      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <Button
            ...
            size="icon"
            className={cn(
              "flex items-center justify-center w-8 h-8 rounded-sm hover:bg-muted/50 transition-all duration-200 !m-0",
              className
            )}
    ...
    <div className="overflow-x-auto hide-scrollbar flex items-center justify-between h-[35px] px-4 bg-muted/50">
      ...
      <div className="flex items-center space-x-3 rtl:space-x-reverse">
        ...
        <div className="w-4 h-4 rounded-sm flex items-center justify-center" title={`Language: ${language}`}>
          {languageIcons[language] || <FileCode className="w-4 h-4" />}
        </div>
        {customActions}
      </div>
    </div>
